		...
		<div class="widget">
			...
			<div class="title chonngonngu">
				<ul>
					...
					<?php } ?>
					<?php if($config_brand=='true'){ ?>
						<div class="formRow">
							<label>Thương Hiệu</label>
							<div class="formRight">

								<select id="thuonghieu" name="thuonghieu" class="main_select select_danhmuc">
									<option value="">Chọn thương hiệu</option>
									<?php for ($i=0;$i<count($thuonghieu); $i++) { ?>
										<option value="<?=$thuonghieu[$i]['id']?>" <?php if($thuonghieu[$i]['id']==$item['thuonghieu']){ echo 'selected="selected"';}?> ><?=$thuonghieu[$i]['ten_vi']?></option>
									<?php } ?>
								</select>
							</div>
							<div class="clear"></div>
					...
					<div class="formRow">
						<label>Giá bán</label>
						<div class="formRight">
							<input type="text" name="giamoi" title="Nhập giá bán" id="giamoi" class="conso tipS validate[required]" value="<?=@$item['giamoi']?>" />
						</div>
						<div class="clear"></div>
					</div>
