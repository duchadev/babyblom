	...
	function delete(){
		...
		if(isset($_GET['id'])){
			...
			for ($i=0 ; $i<count($listid) ; $i++){
				...
				$id =  themdau($idTin);		
				$d->reset();
				$sql = "select id from #_newsletter where id='".$id."'";
				$d->query($sql);
				if($d->num_rows()>0){
			...
			redirect("index.php?com=newsletter&act=man&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
			} else {
			transfer("Không nhận được dữ liệu", "index.php?com=newsletter&act=man&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
		}
