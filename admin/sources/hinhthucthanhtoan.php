...
@define('_table','hinhthucthanhtoan');
switch($act){
	
	case "man":
		get_items();
		$template = "hinhthucthanhtoan/items";
		break;
	case "add":
		$template = "hinhthucthanhtoan/item_add";
		break;
	case "edit":		
		get_item();	
		$template = "hinhthucthanhtoan/item_add";
...
function get_item(){
	...
	$sql = "select * from #_hinhthucthanhtoan where id='".$id."'";
	$d->query($sql);
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			$sql = "delete from #_hinhthucthanhtoan where id='".$id."'";
			$d->query($sql);
		}
