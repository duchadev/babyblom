<?php  if(!defined('_source')) die("Error");
	...
	if($id!=''){
		...
		$d->reset();
		$sql= "select ten_$lang,tenkhongdau,id from #_product_list where hienthi=1 and type='".$type_bar."' and id='".$row_detail['id_list']."'";
		$d->query($sql);
		$row_list = $d->fetch_array();
		
		$d->reset();
		$sql= "select ten_$lang,tenkhongdau,id from #_product_cat where hienthi=1 and type='".$type_bar."' and id='".$row_detail['id_cat']."'";
		$d->query($sql);
		$row_cat = $d->fetch_array();
		
		$d->reset();
		$sql = "select id,thumb,ten_$lang,giamoi,tenkhongdau,giacu,photo from #_product where hienthi=1 and type='".$type_bar."' and banchay=1 order by stt,ngaytao desc";
		$d->query($sql);
		...
		$d->reset();
		$sql = "select * from #_product where hienthi=1 and type='".$type_bar."' and id_list='".$row_detail['id_list']."' and id!='".$row_detail['id']."' order by stt,ngaytao desc";
		$d->query($sql);
		...
		$per_page = $row_setting['product_paging']; // Set how many records do you want to display per page.
		$startpoint = ($page * $per_page) - $per_page;
		$limit = ' limit '.$startpoint.','.$per_page;
		
		$where = " #_product where hienthi=1 and type='".$type_bar."' and id_list='".$row_detail['id']."'  order by stt,ngaytao desc";
		
		$sql = "select ten_$lang,id,thumb,mota_$lang,giacu,giamoi,tenkhongdau,rate,photo from $where $limit";
		$d->query($sql);
		$product = $d->result_array();
		...
		$d->reset();
		$sql = "select id,ten_$lang,tenkhongdau from #_product_list where hienthi=1 and type='".$type_bar."' and tenkhongdau='".$idk."'";
		$d->query($sql);
		$row_list = $d->fetch_array();
		
		$d->reset();
		$sql = "select id,ten_$lang,tenkhongdau,id_list,photo from #_product_cat where hienthi=1 and type='".$type_bar."' and tenkhongdau='".$idc."' and id_list='".$row_list['id']."'" ;
		$d->query($sql);
		$row_detail = $d->fetch_array();
		...
		$d->reset();
		$sql = "select id,ten_$lang,tenkhongdau,photo from #_product_item where hienthi=1 and type='".$type_bar."' and tenkhongdau='".$idi."'";
		$d->query($sql);
		$row_detail = $d->fetch_array();
		...
		if($_GET['thongtin']!=''){
			...
			if($thongtin_tk=='hang-moi-ve'){
				...
				$where_tk  = " and giamoi>='".$where_tk[0]."' and giamoi<='".$where_tk[1]."' ";
			}
		}
