...
function get_gia1($giacu,$giamoi){
	global $res;
	if(($giacu>0) & ($giamoi>0)){
		$res="<span class='giacu'> ".number_format($giacu). "VNĐ</span> <span class='giamoi'>" .number_format($giamoi). " VNĐ</span>";
	}else{
		if($giacu>0){
			$res="<span class='giamoi'>" .number_format($giacu). " VNĐ</span>";
		}else{
			if($giamoi>0){
				$res="<span class='giamoi'>" .number_format($giamoi). " VNĐ</span>";
			}else{
				$res="<a href='lien-he.html' title='Liên hệ'>Liên hệ để có giá tốt nhất !</a>";
			}
...
function get_gia2($giacu,$giamoi){
	global $res;
	if(($giacu>0) & ($giamoi>0)){
		$res="<span class='giacu'> ".number_format($giacu). "VNĐ</span></br> <span class='giamoi'>" .number_format($giamoi). " VNĐ</span>";
	}else{
		if($giacu>0){
			$res="<span class='giamoi'>" .number_format($giacu). " VNĐ</span>";
		}else{
...
function getInlineCssInfo($data){
	...
	foreach($_SESSION['cart'] as $k=>$v){
		...
		$cart_ct.="<tr><td>".$pname." [".$info['masp']."]"."</td><td>".$q."</td><td>".$price."</td><td>".$tt_price."</td></tr>";
	}
	$message.=$cart_ct;
...
function showProduct($v,$options=array(),$k){
	...
	if(!isset($options['class'])){
		$options['class'] = 'col-xs-12 col-sm-6 col3 col-md-3 col-lg-3 item-product	';
	}
	...
	echo '<div class="xm-image"><div class="x-inner"><a href="san-pham/'.$v['tenkhongdau'].'-'.$v['id'].'.html" title="'.$v['ten_'.$lang].'" class=""><img  src="'._upload_product_l.$v['photo'].'" onerror="'.$error.'" class="img-responsive" alt="'.$v['ten_'.$lang].'" /><div class="hover-shine"></div></a></div></div>';
	
	echo '<div class="product-desc anim-05">';
	echo '<h2><<a href="san-pham/'.$v['tenkhongdau'].'-'.$v['id'].'.html" title="'.$v['ten_'.$lang].'" class="">'.$v['ten_'.$lang].'</a></h2>';
	...
	if(($v['giacu'] > 0) & ($v['giacu'] > $v['giamoi']) & $v['giamoi'] > 0){
		echo '<div class="old-price pull-left">';
		echo '<span class="blue">'.myformat($v['giacu']).'</span>';
		echo '</div>';
	}
	echo '<div class="price ">'._gia.' : ';
	echo '<span class="red">'.(($v['giamoi']) ? myformat($v['giamoi']) :' <a href="lien-he.html" title="Liên hệ">Liên hệ</a></span>').'</span>';
	echo '</div>';
...
function sendEmail($to, $from=null, $from_name=null, $subject, $body,$more=array(),$file='') { 
	...
	function phanquyen_tv_cu($com,$quyen,$act,$type){
		global $d;
	...
	function phanquyen_tv($com,$quyen,$act,$type){
		...
		$d->reset();
		$sql = "select * from #_com where ten_com='".$com."' and act ='list' and type ='".$type."'";
		$d->query($sql);
		$getDm = $d->fetch_array();
		...
		if(in_array($act,$xem_arr)){
			if(in_array($com_manager['id'].'|1',$xem_s)){
				if($getDm["danhmuc"] == 1) {
					if($phanquyen["id_list"] != ""){
						if($_GET["id"] != "" || $_REQUEST["id_list"] != "") {
							if(in_array($_GET["id"], json_decode($phanquyen["id_list"])) || in_array($_REQUEST["id_list"], json_decode($phanquyen["id_list"]))) {
								return 1;
							} else {
								return 0;	
							}
	...
	function upload_photos($file, $extension, $folder, $newname=''){
		...
		function make_thu($time){
			$thu['vi'] = array('Chủ nhật','Thứ hai','Thứ ba','Thứ tư','Thứ năm','Thứ sáu','Thứ bảy');
			$str = $thu['vi'][date('w',$time)];
			return $str;
		}
